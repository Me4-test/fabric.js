...
interface IEvent {
  ...
  selected?: Object[];
  deselected?: Object[];
  // transform?: { corner: string, original: Object, originX: string, originY: string, width: number, target?: fabric.Object };
}
...
export class StaticCanvas {
  ...
  default_setViewportTransform(vpt: number[]): Canvas;
...
export class Canvas {
  ...
  _setupCurrentTransform(e: Event, target: fabric.Object, alreadySelected: boolean);
  _transformObject(e: Event);
  _checkTarget(pointer: Point | { x: number, y: number }, obj: Object, globalPointer: Point | { x: number, y: number });
  _normalizePointer(object: Object, pointer: Point | { x: number, y: number })

  _setCursorFromEvent(e: Event, target: Object);

  _currentTransform: ITransform;

  canObjectSelect(object: Object, e: Event): boolean;
  canObjectGroup(object: Object, e: Event): boolean;
}
...
export class Object {
  ...
  isTripleClick(newPointer: Point);
  _stopEvent(e: Event)

  default_render(ctx: CanvasRenderingContext2D);
  _renderBackground(ctx: CanvasRenderingContext2D);

  needCustomUniformToggled: boolean;
  customUniformToggled: boolean;
...
interface ITransform {
  ...
  actionHandler?: (e: Event, transform: ITransform, x: number, y: number) => boolean,
  corner?: number | string | any,
  scaleX?: number;
  scaleY?: number;
  skewX?: number;
  skewY?: number;
...
export class Path {
   ...
   * @param [options] Options object
   */
  constructor(path?: Array<Array<string | number>>, options?: IPathOptions);
...
export class IText extends Text {
  ...
  onMouseDown(options: IEvent);
  mouseUpHandler(options: IEvent);
  isTripleClick(newPointer: Point);
  _stopEvent(e: Event);
  onInput(e?: Event);
}
...
export class Textbox extends IText {
  ...
  customKeyDown(e: KeyboardEvent);
  customKeyUp(e: KeyboardEvent);
  customTextareaAttributes(hiddenTextarea: HTMLTextAreaElement);
  handleInput(hiddenTextarea: HTMLTextAreaElement);

  lastHeight?: number;
  __lastSelected?: boolean;
  __isMousedown?: boolean;

  doubleClickHandler(options: IEvent);
  tripleClickHandler(options: IEvent);
}
...
interface IUtilDomEvent {
   ...
   */
  addListener(element: HTMLElement | Document, eventName: string, handler: Function): void;
